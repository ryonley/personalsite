...
/*
        ...
        $data = $db->query("SELECT * FROM users WHERE id = ?", array(1));
        ...
        //$model = new UserModel();
        $tgservice = $this->serviceLocator->get('table-gateway');
        $model = $tgservice->get('users');

        $rowset = $model->select(array('id' => 1));
        $row = $rowset->current();
